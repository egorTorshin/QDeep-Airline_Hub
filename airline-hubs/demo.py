...
from dimod import ConstrainedQuadraticModel, BinaryQuadraticModel, cqm_to_bqm
...
def build_cqm(W, C, n, p, a, verbose=True):
    ...
    # Constraint: Every leg must connect to a hub (Заменяем квадратичные ограничения на линейные)
    for i in range(n):
        for j in range(n):
            if i != j:
                c1 = BinaryQuadraticModel('BINARY')
                c1.add_linear((i, j), 1)  # Линейное ограничение вместо квадратичного
                c1.add_linear((j, j), -1)  # Заменили на линейное
...
def visualize_results(city_names, hubs, legs, city_lats, city_longs, cost, filenames=None, counter=0, verbose=True):
    ...
    nx.draw_networkx_nodes(hubs, node_size=[v * 100 for v in hub_degrees.values()], pos=positions, node_color='r',
                           edgecolors='k', ax=ax)
...
if __name__ == '__main__':
    ...
    # Преобразуем CQM в BQM с помощью функции из dimod
    bqm, _ = cqm_to_bqm(cqm)  # Возьмем только сам BQM

    # Теперь сэмплируем с использованием Simulated Annealing
